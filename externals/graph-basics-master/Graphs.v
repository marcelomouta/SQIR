...
Fixpoint GV_list {v : V_set} {a : A_set} (g : Graph v a) {struct g} :
 V_list :=
  match g with
  | G_empty => V_nil
  | G_vertex v' a' g' x _ => x :: GV_list g'
  | G_edge v' a' g' x y _ _ _ _ _ => GV_list g'
  | G_eq v' _ a' _ _ _ g' => GV_list g'
  end.
...
Fixpoint GA_list {v : V_set} {a : A_set} (g : Graph v a) {struct g} :
 A_list :=
  match g with
  | G_empty => A_nil
  | G_vertex v' a' g' x _ => GA_list g'
  | G_edge v' a' g' x y _ _ _ _ _ =>
      A_ends x y :: A_ends y x :: GA_list g'
  | G_eq v' _ a' _ _ _ g' => GA_list g'
  end.
...
Fixpoint GE_list {v : V_set} {a : A_set} (g : Graph v a) {struct g} :
 E_list :=
  match g with
  | G_empty => E_nil
  | G_vertex v' a' g' x _ => GE_list g'
  | G_edge v' a' g' x y _ _ _ _ _ => E_ends x y :: GE_list g'
  | G_eq v' _ a' _ _ _ g' => GE_list g'
  end.
...
red; intros Ha; inversion Ha; elim H0; trivial.
